...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        self.ts = self.frame_count / self.fps_vid
        if self.ts > 60: 
            self.ts_min = self.ts // 60 
            self.ts_sec = self.ts%60
        else: 
            self.ts_min = 0 
            self.ts_sec = int(self.ts)
        out_str = f" detected at {self.ts_min} and {self.ts_sec} "

        return path, img, img0, self.cap,out_str
    ...
    def new_video(self, path):
        ...
        self.ts_min = 0 
        self.ts_sec = 0
        self.fps_vid = self.cap.get(cv2.CAP_PROP_FPS)
